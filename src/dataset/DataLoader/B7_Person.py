...
from src.dataset.utils import PERSON_ACTION_TO_ID, filter_by_ids
import sys
...
class B7PersonTempDataset(Dataset):
    def __init__(self, image_root, annot_path, transform=None):
        ...
        for video_id, clips in data.items():
            for clip_id, clip_info in clips.items():
                ...
                self.samples.append(
                    (video_id, clip_id, img_paths, sorted_frames, frame_boxes)
                )
    ...
    def __getitem__(self, idx):
        ...
        # Collect per-player crop sequences
        # Also track the LAST seen label per player
        last_labels = [None] * num_persons

        for t_idx, img_path in enumerate(img_paths):
            fid = sorted_frames[t_idx]
            ...
            for pid in range(num_persons):
                ...
                if box_info is None:
                    ...
                    last_labels[pid] = PERSON_ACTION_TO_ID.get(
                        box_info.action.lower(), 0
                    )
        ...
        labels = [l if l is not None else 0 for l in last_labels]
...
def collate_fn(batch):
    """Stack a batch; flatten (B, N) labels to (B*N,) """
...
def build_loaders(cfg):
    ...
    train_dataset = B7PersonTempDataset(
        data_cfg["videos_path"],
        data_cfg["annot_path"],
        transform=train_transform,
    )
    val_dataset = B7PersonTempDataset(
        data_cfg["videos_path"],
        data_cfg["annot_path"],
        transform=val_transform,
    )
    test_dataset = B7PersonTempDataset(
        data_cfg["videos_path"],
        data_cfg["annot_path"],
