...
def evaluate(model, test_loader, num_classes, device, path):
    ...
    f1_metric_weighted = F1Score(task="multiclass", num_classes=num_classes, average="weighted").to(device)
    ...
    with torch.no_grad():
        for *_, imgs, labels in tqdm(test_loader, leave=False):
            ...
            f1_metric_weighted.update(preds, labels)
    ...
    f1w = f1_metric_weighted.compute().item()
    ...
    log.info(f"Test  acc={acc:.4f}  macro_f1={f1:.4f}  weighted_f1={f1w:.4f}")
    log.info(f"Confusion Matrix:\n{cm}")

    return {"acc": acc, "f1": f1, "weighted_f1": f1w, "confusion_matrix": cm}